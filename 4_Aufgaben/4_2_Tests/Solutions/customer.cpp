...
BOOST_AUTO_TEST_CASE( test_customer )
{
  Account a{ 25 };
  a.deposit( 75 );
  a.deposit( 50 );
  BOOST_TEST(a.balance() == 150);
  BOOST_TEST(a.withdraw( 25 ) == 25);
  BOOST_TEST(a.balance() ==  125);
  BOOST_TEST(a.withdraw( 150 ) == 0);

  Account b{ a };
  BOOST_TEST( !a.transfer( 50, a));
  BOOST_TEST( !a.transfer( 126, b ));
  BOOST_TEST( a.transfer( 125, b ));
  BOOST_TEST( a.balance() == 0);
  BOOST_TEST( b.balance() == 250);

  BOOST_TEST( b.balance() == 300., boost::test_tools::tolerance(100.));
}